...
public interface BookingRepository extends JpaRepository<Booking, String> {
    ...
    @Query("SELECT b.timeSlot, COUNT(b) FROM Booking b WHERE b.date = :date AND b.status IN ('confirmed','active') GROUP BY b.timeSlot")
    List<Object[]> countBookedSlotsByDate(@Param("date") String date);

    @Query("SELECT b.timeSlot, COUNT(b) FROM Booking b WHERE b.date = :date AND b.consoleType = :type AND b.status IN ('confirmed','active') GROUP BY b.timeSlot")
    ...
    @Query(value = "SELECT COUNT(*) FROM bookings WHERE console_id = :consoleId AND date = :date AND time_slot = :timeSlot AND status IN ('confirmed','active')", nativeQuery = true)
